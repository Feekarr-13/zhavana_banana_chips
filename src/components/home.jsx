...
const Home = () => {
  ...
  // 🟢 Efek animasi awal (hanya sekali saat mount)
  useEffect(() => {
    ...
    heroImgTl.fromTo(
      ".imgHero",
      { scale: 1, y: 0, duration: 10, delay: 5 },
      { scale: "1.02", y: -5, repeat: -1, yoyo: true }
    );
  }, []);

  // 🟢 Efek yang responsif terhadap ukuran window
  useEffect(() => {
    const handleResize = () => {
    ...
    if (lebarWin <= 930) {
      gsap.to(".scrollAnim", { opacity: 0 });
    } else {
      gsap.fromTo(
        ".scrollAnim",
        { duration: 0.3, y: 10, opacity: 0 },
        { y: 0, opacity: 1 }
      );
    }
  ...
  return (
    <div
      className={`${style.paddingX} w-full h-fit sl:h-screen bg-primary text-txtcolor`}
    >
      <div
      ...
      >
        ...
        <div className="heroContent flex flex-col mb-10 md:pt-10">
          ...
          <div className="buttons">
            <a
              href={getWaApi()}
              target="blank"
              className={`${style.btnWhite} inline-block mr-3 sm:mr-5`}
            >
              Order Sekarang
            </a>
            <a
        ...
        <a
          href="#produk"
          className="scrollAnim w-[29px] h-[67px] rounded-[19px] border-2 border-secondary absolute bottom-5 inset-x-1/2 py-1 flex justify-center opacity-0 sl:opacity-100"
        >
